...
configspath 	from inspect import getmembers, isfunction, ismodule
= 'configs/'
...
if __name__=='__main__':
	...
	omega_smg= expansion_smg[0]
	w_0=expansion_smg[1]
	w_a=expansion_smg[2]
	print(omega_smg,w_0,w_a)
	...
	parameters = [
		{
			...
			"derivative_args": lambda x: Cl_func_wrapped(cosmo_params, H0_true, Omega_m_true, Omega_b_true, A_s, n_s,
														 alpha_M, alpha_B, omega_smg, x, w_a),
			"key": "der_w_0_cov_mat"
		},
		{
			"name": "w_a",
			...
			"step": 1e-3,
			"derivative_args": lambda x: Cl_func_wrapped(cosmo_params, H0_true, Omega_m_true, Omega_b_true, A_s, n_s,
														 alpha_M, alpha_B, omega_smg, w_0, x),
			"key": "der_w_a_cov_mat"
		},
		{
			...
			"true_value": H0_true,
			"step": 1e-2,
			"derivative_args": lambda x: Cl_func_wrapped(cosmo_params, x, Omega_m_true, Omega_b_true, A_s, n_s, alpha_M,alpha_B,omega_smg, w_0, w_a),
			"key": "der_H0_cov_mat"
		},
		{
	...
	# _, P_vals_camb = C.camb_Pk(z=zz_nl, k=kk_nl, nonlinear=True, halofit='mead2020')
	...
	if Lensing:
		...
		def Cl_func(C, params, gw_params, dl_GW, bin_edges_dl, z_gal, ll, b_gal, b_GW, save, n_points=13, n_points_x=20,
					grid_x='lin', z_min=1e-05, n_low=5, n_high=5):
			...
			# _, P_vals_camb = C.camb_Pk(z=zz, k=kk, nonlinear=True, halofit='mead2020')
			# print('\n')
			# print((P_vals_camb.T / pkz) - 1)
			# print('\n')
			#print('background:\n',bg)
			...
			# start = time.time()
			Cl = S.limber_angular_power_spectra(bg,l=ll, windows=['galaxy', 'GW', 'rsd', 'lsd'])
			# end = time.time()
	...
	# If lensing is not included, compute only density clustering spectra
		...
		def Cl_func(C, params, gw_params, dl_GW, bin_edges_dl, z_gal, ll, b_gal, b_GW, save, n_points=13, n_points_x=20,
			...
			Cl = S.limber_angular_power_spectra(bg,l=ll, windows=None)
	...
	Cl_GG, Cl_GWGW, Cl_GGW = Cl_func(C, cosmo_params, gw_params, dl_GW, bin_edges_dl, z_gal, ll, b_gal=bias_gal,
									 b_GW=bias_GW, save=True)
	...
	def Cl_func_wrapped(cosmo_params, H0, Omega_m, Omega_b, A_s, n_s, alpha_M, alpha_B,omega_smg, w_0, w_a):
		"""
		...
		params['expansion_smg'] = f"{omega_smg},{w_0},{w_a}"
		...
		return Cl_func(C, params, gw_params, dl_GW, bin_edges_dl, z_gal, ll, bias_gal, bias_GW,save=False)
	...
	def compute_partial_derivatives_gal(b_gal, bias_GW, der_b_gal, step):
		...
		for i in range(len(b_gal)):
			...
			def func_GG(b):
				...
				b_gal_temp[i] = b
				return Cl_func(C, cosmo_params, gw_params, dl_GW, bin_edges_dl, z_gal, ll, b_gal_temp,
	...
	def compute_partial_derivatives_GW(b_GW, bias_gal, der_b_GW, step):
		...
		for i in range(len(b_GW)):
			...
			def func_GG(b):
				...
				b_GW_temp[i] = b
				return \
				Cl_func(C, cosmo_params, gw_params, dl_GW, bin_edges_dl, z_gal, ll, bias_gal, b_GW_temp, save=False)[0]

			def func_GWGW(b):
				b_GW_temp = np.copy(b_GW)
				b_GW_temp[i] = b
				return \
				Cl_func(C, cosmo_params, gw_params, dl_GW, bin_edges_dl, z_gal, ll, bias_gal, b_GW_temp, save=False)[1]

			def func_GGW(b):
				b_GW_temp = np.copy(b_GW)
				b_GW_temp[i] = b
				return \
				Cl_func(C, cosmo_params, gw_params, dl_GW, bin_edges_dl, z_gal, ll, bias_gal, b_GW_temp, save=False)[2]
	...
	sigma_H0, sigma_omega, sigma_omega_b, sigma_As, sigma_ns, sigma_alpha_M, sigma_alpha_B, sigma_w_0, sigma_w_a = fem.compute_and_print_sigma_params(
		fisher_inv)
	...
	with open(os.path.join(FLAGS.fout, 'results.txt'), 'a') as file:
		file.write(
			...
			% (GW_det, yr, Lensing, bin_strategy, n_bins_z, n_bins_dl, z_m_bin, z_M_bin, sig_gal, l_max, n_gal_bins,
			   n_GW_bins, rel_err_H0, rel_err_omega, rel_err_alpha_M, rel_err_alpha_B))
